...
void log_time_comparison_at_pps(const char* filename) {
    ...
    if (s_calibration_active) {
        if (s_calib_start_pps == 0) {
            ...
            ESP_LOGI(GPS_REDO_TAG, "Calibration started (target %d s window). PPS start=%llu", CALIBRATION_DURATION_PPS, (unsigned long long)s_calib_start_pps);
            ...
            ESP_LOGI(GPS_REDO_TAG, "Calibration complete: XTAL drift total=%lld us over %llu s => %.3f ppm; DS drift total=%lld us => %.3f ppm", (long long)xtal_drift_total, (unsigned long long)delta_pps, s_xtal_ppm_correction, (long long)(ds_valid? (ds_offset_us - s_calib_ds_offset0):0), s_ds_ppm_correction);
        }
    }
    ...
    ESP_LOGI(GPS_REDO_TAG, "%sLOG: PPS=%llu XTAL_off=%lld DS_off=%lld driftX=%lld driftD=%lld ppmX=%.3f ppmD=%.3f (mode=%s) HERE",
