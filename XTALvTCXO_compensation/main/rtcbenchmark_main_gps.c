...
static volatile uint64_t last_callback_time = 0;  // last ISR time
...
static volatile int64_t last_pps_sqw_phase_us = 0;
...
// PPS-boundary captured slow-clock and count 
...
// expose latest PPS–SQW phase for logging elsewhere
...
static bool IRAM_ATTR pps_hw_capture_callback(mcpwm_cap_channel_handle_t cap_chan, const mcpwm_capture_event_data_t *edata, void *user_data){
    ...
    // update PPS–SQW phase using 80 MHz capture ticks 
    if (has_sqw_cap) {
        // compute signed delta ticks
    ...
    // notify logger every 60th PPS after sync
...
// SQW capture ISR: record timestamp and count
...
void analyze_hardware_captures(void) {
    // take a consistent snapshot of the ring buffer indices and the last samples
    uint32_t local_ci;
...
void app_main(void)
{
    ...
    // Give bus/device a moment to settle before first transaction (in case of power-up lag)
    ...
    // start PPS-boundary periodic logger (preferred for drift correctness)
